        ...
        function   obj = infer(obj, param)
            ...
            if crossvalid
                ...
                for p=1:nSet % for each permutation
                    ...
                    % store weights for each CV set
                    %                     for m=1:nM
                    %                         for d=1:M(m).nDim
                    %                             for r=1:M(m).rank
                    %                                 obj.regressor(m).Weights(d).U_CV{r}(p,:) = obj_train.regressor(m).Weights{d}.U(r,:);
                    %                             end
                    %                         end
                    %                     end
            ...
            elseif do_grad_hyperparam % optimize parameters directly on whole dataset
                ...
                error('check all these equations again');
                ...
                for q=1:size(CovJacob,3) % for each hyperparameter
                    ...
                    gradgrad = [allU{:}]*CovJacob(:,:,q)*PP'; % LLH derived w.r.t to U (free basis) and hyperparameter
                    gradU = - PP' * Hinv * gradgrad';% derivative of inferred parameter U w.r.t hyperparameter (full parametrization)
...
while ~isempty(fmla)
        ...
        while fmla(1)~=')'
            ...
            switch lower(option)
                ...
                case 'basis'
                    ...
                    [opts.basis, fmla] = starts_with_word(fmla, ...
                        {'poly([\d]+)(', 'exp([\d]+)(', 'raisedcosine([\d]+)('});
                ...
                case 'period' % define period
                    i = find(fmla==')' | fmla==',',1); % find next coma or parenthesis
                    if isempty(i)
    ...
    if fmla(1)==')'
        pp = find(isnan(CloseBrackPos),1,'last'); % check last open bracket which has not been closed yet
        assert(~isempty(pp) || withMixture, 'there is no open parenthesis to close');
        if ~isempty(pp)
            CloseBracketPos(pp) = length(O);
        end
        fmla = trimspace(fmla(2:end));
    end
...
[M, idxC] = compose_regressors(V,O, OpenBracketPos, CloseBracketPos);
...
%         O(v) = [];
%
%     else % now we're left with additions and substractions
%         if O(1)=='-' % substraction
%             V{2} = V{2}*(-1);
%         end
%
...
function [M, idxC] = compose_regressors(V,O, OpenBracketPos, CloseBracketPos)


%% process parentheses first
while ~isempty(OpenBracketPos) % as long as there are parenthesis to process
...
if twodmap
    % color plot
    h_nu = colorplot(scale{1}(:,1),scale{1}(:,2), U);
elseif imageplot
    % image plot
    h_nu = imagesc(scale{2},scale{1}, U);
else
