...
% - 2: asymmetrical lapses (one parameter for each response) [default]
%
...
% procedure (default:1 if model without lapse, as porblem is convex; 100 otherwise)
...
%     - 'LLH_iter_all':  cell with the vectors of lost-posterior at each minimization step,
...
classdef mixture
    % add HELP
    %
    % M = mixture(nC, nObs,'multinomial')
    % M = mixture(nC, nObs,'multinomial', Cat)
    %
    % M = mixture(nC, nObs,'regression', X)
    %
    % M = mixture(nC, nObs,'hmm')
    %
    % M = mixture(nC, nObs,PosteriorFun, ParameterFittingFun, InitialPars)
    % M = mixture(nC, nObs,PosteriorFun, ParameterFittingFun, InitialPars, label)
    properties
        nComponent
        nObservations
        Data
    ...
    methods
        ...
        %% CONSTRUCTOR %%%%%
        function obj = mixture(nC, nObs, varargin)
            assert(isscalar(nC) && nC>1, 'nComponent must be a positive integer');
            obj.nComponent = nC;

            assert(isscalar(nObs) && nObs>1, 'nComponent must be a positive integer');
            obj.nObservations = nObs;

            if isstring(varargin{1}) % built-in models

                obj.label = lower(varargin{1});

                switch lower(varargin{1})
                    case 'multinomial' % MULTINOMIAL MODEL
                        if length(varargin)>1
                            obj.Data = varargin{2}(:);
                            assert((length(obj.Data)== nObs) && all(obj.Data>0),...
                                'categorial data must be a vector of positive integers and length nObs');
                        else
                            obj.Data = ones(nObs,nC);
                        end

                        nCat = length(unique(obj.Data)); % number of categories

                        % initial values (uniform distribution between all
                        % components)
                        obj.Parameters = ones(nCat, nC)/nC;

                        obj.PosteriorFun = @MultinomialPosterior;
                        obj.ParameterFitting = @MultinomialFitting;

                    case 'regression' % REGRESSION MODEL
                        assert(length(varargin)>1, 'missing regressors for regression mixture model');
                        obj.Data = varargin{2};
                        assert(size(obj.Data,1)==nObs, 'the regression data must have nObs rows');
                        nWeights = size(obj.Data,2); % number of columns

                        % initial values for parameters(uniform distribution between all
                        % components)
                        obj.Parameters = zeros(nWeights-1, nC);

                        obj.PosteriorFun = @MnrPosterior;
                        obj.ParameterFitting = @MnrFitting;
                    case 'hmm'

                        % initial values for parameters (uniform transition probabilities between all
                        % components and uniform value)
                        obj.Parameters = ones(nC+1, nC)/nC; % first row is for initial valus

                        obj.PosteriorFun = @HMMPosterior;
                        obj.ParameterFitting = @HMMFitting;

                    otherwise
                        error('incorrect model type: %s',varargin{1});
                end

            else % user-defined model
                obj.PosteriorFun = varargin{1};
                obj.ParameterFitting = varargin{2};
                obj.Parameters = varargin{3};
                if length(varargin)>3
                    obj.label = varargin{4};
                end
...
if ninit>1 && size(x0,2)<2
    ...
    if do_lapse
        if ngating>0
            ...
            lapse = gamrnd(repmat(alpha',ninit-1,1),1,ninit-1,1+nlapse); % initial lapse value: sample from prior Dirichlet distribution
...
% all_x = zeros(npar,ninit);
% all_lostpost = zeros(1,ninit);
% all_LLH_MAP = zeros(1,ninit);
% all_LLH_iter = cell(1,ninit);
% all_exitflag = zeros(1,ninit);
% all_niter =  zeros(1,ninit);
%
% for n = 1:ninit
%
%     w = x0(1:nvar,n); % weights for each variable
...
%          yy, yy_s, Ridge, Ridgemat, alpha, nlapse, do_lapse, FR,logB, param);
%
%     %%%%% AFTER ITERATION PROCEDURE
%     all_x(:,n) = [w;lapse(1:nlapse*do_lapse)];
%     all_exitflag(n) = converged;
%     all_niter(n) = iter;
%     all_lostpost(n) = logpost;
%     all_LLH_MAP(n) = LLH;
%     all_LLH_iter{n} = logpost_iter;
% end
%
% % find optimization procedure that yielded the best overall LLH
% [logpost,imax] = max(all_lostpost);
% LLH_MAP = all_LLH_MAP(imax);
% x = all_x(:,imax);
% w = x(1:nvar); % weights for each variable
% if do_lapse
%     lapse = x(i_lapse); % lapse
% else
%     lapse = lapse';
% end
% exitflag = all_exitflag(imax);
% logpost_iter = all_LLH_iter{imax};
...
if exitflag>0 && all(Ridge>0)
               logdetH = 2*sum(log(diag(chol(-H)))); % log-determinant of -H (fast reliable way)
               S.LLH = logpost - logdetH/2   +npar/2*log(2*pi);

    %     if nlapse==2, % Dirichlet prior
    %         S.LLH = S.LLH-2;
    %     end
else
    S.LLH = nan;
end
S.BIC = LLH_MAP - npar/2*log(nobs); % Bayes Information Criterion
...
S = rmfield(S,'all_x');
%S.LLH_all = SS.all_logpost;
%S.exitflag_all = SS.all_exitflag;
%S.LLH_iter_all = SS.all_LLH_iter;
...
while still
    ...
    if do_lapse
        ...
        %         switch nlapse
        %             case 1
        %                 pi_m = 1 -2*lapse; % prior probability of no-laspe
        %                 lh = lapse + pi_m*Pm; % likelihood of response 1
        %             case 2
        %                 pi_m = 1 - sum(lapse);
        %                 lh = lapse(2) + pi_m*Pm;
...
%                 lapse = reshape(lapse,ngating,2);
    ...
    %         case 0
    %             lh = Pm;
    %             pi_m = 1; % prior probability of no-laspe
    %         case 1
    %             pi_m = 1 -2*lapse; % prior probability of no-laspe
    %             lh = lapse + pi_m*Pm; % likelihood of response 1
    %         case 2
    ...
    if isempty(param.gating)
        switch nlapse*do_lapse
            case 1 % beta prior for symmetrical lapses
                ...
                logpost = logpost + sum( (alpha-1).*log(fullapse)) - logB;
    ...
    converged = abs(old_logpost-logpost)<param.epsilon;
    old_logpost = logpost;
